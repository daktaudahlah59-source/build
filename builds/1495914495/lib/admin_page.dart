...
class _AdminPageState extends State<AdminPage> {
  ...
  final List<String> roleOptions = ['reseller', 'member', 'reseller', 'vip'];
  ...
  Future<void> _loadConfigAndFetch() async {
    ...
    try {
      ...
      _alert("⚠️ Config Error", "Gagal mengambil konfigurasi server.\nError: $e");
  ...
  Future<void> _deleteUser() async {
    if (!configLoaded) {
      _alert("⚠️ Error", "Konfigurasi server belum siap.");
      return;
    }
    ...
    try {
      final res = await http.get(
        Uri.parse(
          'http://capekkenaoanyak.onlinepanel.my.id:2002/deleteUser?key=$sessionKey&username=$username',
        ),
      );
      final data = jsonDecode(res.body);
  ...
  Future<void> _createAccount() async {
    ...
    try {
      ...
      final url = Uri.parse(
        'http://capekkenaoanyak.onlinepanel.my.id:2002/userAdd?key=$sessionKey&username=$username&password=$password&day=$day&role=$newUserRole',
      );
