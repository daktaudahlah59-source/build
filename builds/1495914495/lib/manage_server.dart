...
class _ManageServerPageState extends State<ManageServerPage> {
  ...
  Future<void> _loadConfigAndFetch() async {
    ...
    try {
      ...
      _showError("Gagal mengambil konfigurasi server.\nError: $e");
  ...
  Future<void> _fetchVpsList() async {
    ...
    setState(() => isLoading = true);
    final uri = Uri.parse('http://capekkenaoanyak.onlinepanel.my.id:2002/myServer?key=${widget.keyToken}');
    try {
  ...
  Future<void> _addVps() async {
    if (!configLoaded) {
      _showError("Konfigurasi server belum siap.");
      return;
    }
    ...
    final uri = Uri.parse('http://capekkenaoanyak.onlinepanel.my.id:2002/addServer');
    try {
  ...
  Future<void> _deleteVps(String host) async {
    ...
    final uri = Uri.parse('http://capekkenaoanyak.onlinepanel.my.id:2002/delServer');
    try {
      final res = await http.post(uri, body: {
        'key': widget.keyToken,
        'host': host,
