...
class _VipPageState extends State<VipPage> {
  ...
  final List<String> roleOptions = ['reseller', 'member', 'admin'];
  ...
  Future<void> _loadConfigAndFetch() async {
    ...
    try {
      ...
      _alert("Error", "Gagal mengambil konfigurasi server.\nError: $e");
  ...
  Future<void> _fetchUsers() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('http://capekkenaoanyak.onlinepanel.my.id:2002/listUsers?key=$sessionKey'),
      );
  ...
  Future<void> _deleteUser() async {
    if (!configLoaded) {
      _alert("Error", "Konfigurasi server belum siap.");
      return;
    }
    ...
    try {
      final res = await http.get(
        Uri.parse('http://capekkenaoanyak.onlinepanel.my.id:2002/deleteUser?key=$sessionKey&username=$username'),
      );
      final data = jsonDecode(res.body);
  ...
  Future<void> _createAccount() async {
    ...
    try {
      final url = Uri.parse(
        'http://capekkenaoanyak.onlinepanel.my.id:2002/userAdd?key=$sessionKey&username=$u&password=$p&day=$d&role=$newUserRole',
      );
  ...
  Future<void> _editUser() async {
    ...
    try {
      final url = Uri.parse(
        'http://capekkenaoanyak.onlinepanel.my.id:2002/editUser?key=$sessionKey&username=$u&addDays=$d',
      );
      final res = await http.get(url);
      final data = jsonDecode(res.body);
      ...
      children: List.generate(totalPages, (index) {
        ...
        return ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            ...
            foregroundColor: currentPage == page ? Colors.black : accentPurple,
      ...
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [bgDark, accentPurple.withOpacity(0.1), bgDark],
          ),
        ),
        child: SafeArea(
